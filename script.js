...
/*
        if (month==3||month==5||month==7||month==10 && dayF > 30) {
            sun1.innerHTML += ` (${Number(dayF-30)})` // 1º domingo depois do dia 30
            console.log(dayF)
        } else {
            sun1.innerHTML += ` (${Number(dayF)})`
        }
        if (month==3||month==5||month==7||month==10 && dayF+7 > 30) {
            sun2.innerHTML += ` (${Number(dayF)+7-30})` // 2º dom. depois do dia 30
            console.log(dayF)
        } else {
            sun2.innerHTML += ` (${Number(dayF)+7})`
        }
        if (month==3||month==5||month==7||month==10 && dayF+14 > 30) {
            sun3.innerHTML += ` (${Number(dayF+14-30)})` // 3º dom. depois do dia 30
            console.log(dayF)
        } else {
            sun3.innerHTML += ` (${Number(dayF) + 14})`
        }
*/

/*
function dateCal(year) {
    if (year % 400 == 0 || year % 4 == 0 && year % 100 != 0) {// Se for bissexto(Multiplo de 4 e de 400 mas não de 100)
        if (month == 1 && dayF > 29) { // Fevereiro
            if (dayF > 29) {
                sun1.innerHTML += ` (${Number(dayF - 29)})` // 1º domingo depois do dia 28
            } else {
                sun1.innerHTML += ` (${Number(dayF)})`
            }
            if (dayF + 7 > 28) {
                sun2.innerHTML += ` (${Number(dayF) + 7 - 29})` // 2º dom. depois do dia 28
            } else {
                sun2.innerHTML += ` (${Number(dayF) + 7})`
            }
            if (dayF + 14 > 28) {
                sun3.innerHTML += ` (${Number(dayF + 14 - 29)})` // 3º dom. depois do dia 28
            } else {
                sun3.innerHTML += ` (${Number(dayF) + 14})`
            }
        } else {
        ...
        // Fevereiro (pra que esse mês existe msm?)
        if (month == 1) {
            if (dayF > 28) {
                sun1.innerHTML += ` (${Number(dayF - 28)})` // 1º domingo depois do dia 28
            } else {
                sun1.innerHTML += ` (${Number(dayF)})`
            }
            if (dayF + 7 > 28) {
                sun2.innerHTML += ` (${Number(dayF) + 7 - 28})` // 2º dom. depois do dia 28
            } else {
                sun2.innerHTML += ` (${Number(dayF) + 7})`
            }
            if (dayF + 14 > 28) {
                sun3.innerHTML += ` (${Number(dayF + 14 - 28)})` // 3º dom. depois do dia 28
            } else {
                sun3.innerHTML += ` (${Number(dayF) + 14})`
            }
        }
        ...
        if (month == 0 || month == 2 || month == 4 || month == 6 || month == 7 || month == 9 || month == 11) {
            if (dayF > 31) {
                sun1.innerHTML += ` (${Number(dayF - 31)})` // 1º domingo depois do dia 31
            } else {
                sun1.innerHTML += ` (${Number(dayF)})`
            }
            if (dayF + 7 > 31) {
                sun2.innerHTML += ` (${Number(dayF) + 7 - 31})` // 2º dom. depois do dia 31
            } else {
                sun2.innerHTML += ` (${Number(dayF) + 7})`
            }
            if (dayF + 14 > 30) {
...
function sunday() {
    ...
    if (month == 3 || month == 5 || month == 7 || month == 10) {
        ...
        if (month == 3 || month == 5 || month == 7 || month == 10 && dayF + 14 > 30) {
            sun3.innerHTML += ` (${(dayF + 14 - 30).toString().length == 1 ? '0' + (dayF + 14 - 30): (dayF + 14 - 30)})` // 3º dom. depois do dia 30
        } else {
            sun3.innerHTML += ` (${(dayF + 14).toString().length == 1 ? '0' + (dayF + 14): (dayF + 14)})`
...
if (year % 400 == 0 || year % 4 == 0 && year % 100 != 0) {// Se for bissexto (Multiplo de 4 e de 400 mas não de 100)
    if (month == 1 && dayF > 29) { // Fevereiro
        ...
        if (dayF + 14 > 28) {
            sun3.innerHTML += ` (${((dayF + 14 - 29).toString()).length < 1 ? '0' + dayF: dayF})` // 3º dom. depois do dia 28
        } else {
            sun3.innerHTML += ` (${Number(dayF) + 14})`
        }
    }
...
/* function dayMonth() {
    ...
    if (year % 400 == 0 || year % 4 == 0 && year % 100 != 0) {
        if (month == 1) { // Se for bissexto bla bla
            return 29
        }
    } else {
        return leapYear()
...
function calcular() {
    for (let i = 0; i < 4; i++) {
        console.log('funcionou ' + sun.value)
        if (snack.value == 1) {
            test[0].setAttribute('src', 'hamburguer.jpg')
            style()
            text.innerHTML = `Dia ${dayF} (daqui há ${(year % 400 == 0 || year % 4 == 0 && year % 100 != 0) == true ? dayF - day : dayF - day} dias)` // utilidade do dayF já ser o próximo domingo
        } else {
            if (snack.value == 2) {
                img.setAttribute('src', 'hotdog.jpg')
                style()
                text.innerHTML = `Dia ${dayF} (daqui há ${(year % 400 == 0 || year % 4 == 0 && year % 100 != 0) == true ? dayF - day : dayF - day} dias)`
            }
        }
    }
}
...
/*
texts[i].innerHTML = `Dia ${dayF+s > dayMonth() ? dayF+s-dayMonth(): dayF+s} (daqui há ${(year % 400 == 0 || year % 4 == 0 && year % 100 != 0) == true ? dayF+s - day : dayF+s - day} dias)` // utilidade do dayF já ser o próximo domingo
*/
